...
| `tray.svg` | 菜单栏托盘图标的源文件：两个 S——重影只留轮廓、主体实心，纯黑透明底 |
...
所以源文件里只能有纯黑与透明——重影那三档灰在这儿没有意义。
试过两版：只留主体 S（重影没了就不像这个标志），以及把主体加粗一圈从重影上挖出缝隙
（缝隙贴着主体轮廓走，小尺寸下像把字描歪了，难看）。**现在这版是两个 S**：
重影只留轮廓（26 的描边、不填充），主体实心，两个形状各自成立，小尺寸下也分得开。
改了它之后：
