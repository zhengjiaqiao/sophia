...
/// 删网关延迟提交：标记后状态里看不到，配置和密钥都还在；撤销原样恢复
#[test]
fn marking_a_removal_hides_the_provider_and_undo_restores_it_untouched() {
    let f = fixture();
    let (a, b) = two_providers(&f);
    f.app.enable().unwrap();
    let settings_before = f.world.lock().unwrap().settings.clone();
    let routing_before = f.routing();
    let config_before = f.read_config();

    f.app.mark_removal(&b).unwrap();
    let state = f.app.state();
    assert_eq!(
        state.providers.iter().map(|p| p.id.as_str()).collect::<Vec<_>>(),
        [a.as_str()]
    );
    {
        let world = f.world.lock().unwrap();
        assert_eq!(world.settings, settings_before, "标记不动配置");
        assert!(world.keys.contains_key(&b), "标记不删密钥");
        assert!(world.deleted_keys.is_empty());
    }
    assert_eq!(f.routing(), routing_before);

    f.app.undo_removal(&b);
    let state = f.app.state();
    assert_eq!(state.providers.len(), 2);
    assert_eq!(state.providers[1].id, b);
    assert_eq!(state.providers[1].models.len(), 1);
    assert!(state.providers[1].has_key);
    assert_eq!(f.world.lock().unwrap().settings, settings_before);
    assert_eq!(f.routing(), routing_before);
    assert_eq!(f.read_config(), config_before);

    // 撤销之后再提交全部：没有可删的
    f.app.commit_removals(None).unwrap();
    assert_eq!(f.app.state().providers.len(), 2);
    assert!(f.world.lock().unwrap().deleted_keys.is_empty());
}

/// 提交后配置和密钥都没了；按 id 提交只删那一家，不带 id 提交剩下的全部
#[test]
fn committing_removals_deletes_config_and_key() {
    let f = fixture();
    let (a, b) = two_providers(&f);
    f.app.enable().unwrap();
    f.app.mark_removal(&b).unwrap();
    f.app.commit_removals(Some("not-pending")).unwrap();
    assert!(f.world.lock().unwrap().settings.provider(&b).is_some());

    f.app.commit_removals(Some(&b)).unwrap();
    {
        let world = f.world.lock().unwrap();
        assert!(world.settings.provider(&b).is_none(), "配置删了");
        assert!(!world.keys.contains_key(&b), "密钥删了");
        assert_eq!(world.deleted_keys, std::slice::from_ref(&b));
    }
    assert!(!catalog_slugs(&f).contains(&"other-gateway-deepseek-v4".to_owned()));
    assert_eq!(f.app.state().providers.len(), 1);

    // 撤销已提交的：什么也不发生
    f.app.undo_removal(&b);
    assert_eq!(f.app.state().providers.len(), 1);

    // 没勾模型的一家可以删光；不带 id 的提交（离开网关页 / 退出）删全部标记
    f.app.restore().unwrap();
    f.app.mark_removal(&a).unwrap();
    assert!(f.app.state().providers.is_empty());
    f.app.commit_removals(None).unwrap();
    let world = f.world.lock().unwrap();
    assert!(world.settings.providers.is_empty());
    assert!(!world.keys.contains_key(&a));
}

/// 真删会被拒绝的（已启用且是最后一家在发布模型的）标记时就拒绝；未知 id 报错
#[test]
fn marking_the_last_publishing_provider_while_enabled_is_refused_up_front() {
    let f = fixture();
    let (a, b) = two_providers(&f);
    f.app.enable().unwrap();
    f.app.mark_removal(&a).unwrap();
    assert_eq!(code(f.app.mark_removal(&b)), "invalid", "两家都标记等于删光");
    assert_eq!(f.app.state().providers.len(), 1);
    assert_eq!(code(f.app.mark_removal("nope")), "invalid");
    f.app.mark_removal(&a).unwrap(); // 重复标记不算错
}
