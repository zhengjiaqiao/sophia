...
// DESIGN「刚点亮反色闪 120ms」：第一帧必须是完整反色（ink 底 + face 记号），
...
test("ss-flash：0% 起就是完整反色，保持到 60% 才淡出", () => {
  ...
  assert.match(first[1], /color:\s*var\(--face\)/);
});

test("ss-flash-dot：记号在同一段里转 face（实心点与空心环都跟 currentColor）", () => {
  const body = keyframes("ss-flash-dot");
  assert.match(body, /0%,\s*60%\s*\{[^}]*color:\s*var\(--face\)/);
});
