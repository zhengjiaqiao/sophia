...
- [ ] 「全局」页：通用仓库、Codex、WeiboAP 自有本体位置的全部 skill 成行；WeiboAP custom 目录是托管目录，只作本体位置，全局页没有 WeiboAP 列，这些 skill 可同步到 Claude Code / Codex 等列
...
- [ ] 每个助手各是一个项目域、各有一列；在某个助手的域页面补齐，只有该助手目录下新增软链，其余助手目录不变
...
- [ ] 项目页显示所有已启用 harness 的列：某项目没有 `.agents/skills` 时 Codex 也成列，列头在名字后标「将新建目录」（title 是完整路径）、整列为 ○；点补齐后目录被建出来、链接建立，重扫后该标注消失、其中的链接照常可清除
...
- [ ] 非 macOS（或 `gateway_state` 返回 `supported: false` / 读取失败）：侧栏导航没有「模型」按钮；点不到、也不渲染 ModelsTab
- [ ] macOS 且 `supported: true`：导航里 MCP 之后出现「模型」按钮，点开后从上到下依次是提示条、网关、模型、Codex 四块加底部一句已知限制
- [ ] 切到「模型」页时侧栏域列表不显示 Skills/MCP 的域（「全部」「全局」仍在），点它们不报错、不影响模型页内容
- [ ] `enabled && !router.running`：顶部出现路由不可用提示，文案含 `router.error`；点「恢复」弹出确认框，确认后调用恢复
- [ ] `conflict` 非空：顶部出现该文案的提示条（无按钮）；此时「启用」按钮禁用且旁边文字就是这段冲突原因
- [ ] `takeover` 非空：顶部出现“本机当前由 agents-manager 启用（网关 …，已选 N 个模型），可以由 SymSync 接管”提示，「接管」按钮先弹确认框（说明接管后 Codex 需要重启一次）再调用接管；此时「启用」按钮禁用，原因文字为待接管说明
- [ ] `codex.drift` 为真：顶部出现版本漂移提示，「重新生成」按钮直接调用启用命令（`gateway_enable`），不用二次确认
- [ ] `needsCodexRestart` 为真：顶部出现「需要重启 Codex 才生效」纯文字提示，没有按钮
- [ ] 网关地址输入框可编辑；密钥输入框类型为密码，`hasKey` 为真时占位符是「已保存，留空则不修改」，为假时是「输入密钥」；页面任何状态下都不回显已保存的密钥
- [ ] 「保存」成功后密钥输入框被清空；保存失败时地址与密钥输入原样保留，不清空，错误经顶部错误横幅展示且已去掉 `[code] ` 前缀（`[changed]` 的正文原样展示，提示重试）
- [ ] 「拉取模型」在 `hasKey` 为假或地址为空时禁用；拉取成功后模型列表刷新，失败不改变已保存内容
- [ ] 模型筛选框按 id/slug/displayName 大小写不敏感过滤；已选模型排在最前面；模型总数为 0 时显示空状态文案；有模型但筛选无匹配时显示“没有匹配的模型”
- [ ] 每个模型一行：勾选框 + 可编辑显示名输入框；页面顶部/模型区显示当前已选数量，随勾选实时更新
- [ ] 「保存选择」把当前勾选与显示名一次性提交，成功后用返回的最新状态刷新列表
- [ ] Codex 区状态行正确显示已启用/未启用、后台服务是否安装、运行与端口（未安装时不显示运行/端口）、Codex 版本
- [ ] 「启用」按钮在满足条件时可用；被禁用时旁边显示禁用原因文字，优先级为：待接管 > 冲突 > 未保存密钥 > 未选模型
- [ ] 「恢复」按钮仅在 `enabled` 或 `router.installed` 为真时可用；点击先弹确认框，确认后才调用恢复命令
- [ ] 页面底部固定展示第三方模型的已知限制一句话（会话标题发官方、自动审阅不可用、网页搜索等工具不可用）
- [ ] 所有失败均通过页面顶部既有的错误横幅展示，模型页自身没有第二套提示/横幅机制

## 菜单栏面板（docs/specs/2026-09-21-tray.md，仅 macOS）

自动化测试覆盖不到窗口与系统菜单栏的交互，发版前过一遍：

1. 左键点菜单栏的 S 图标：面板出现在图标正下方；再点一次图标收起；点面板外任何地方也收起。
2. 把菜单栏图标拖到最右侧再点：面板整体留在屏幕内。外接显示器上同样检查一次。
3. 已启用时开关是反色「已启用」；点一下停用，主窗口的「模型」页跟着变。
4. 刚启用或刚停用、Codex 还开着：面板里出现「重启 Codex」；点它先变成一句后果加「重启」「取消」，点「取消」什么都不发生。没有改动等着生效时，面板里没有这一行。
5. 占住路由端口后在面板里点「启用」：主窗口到前面并停在「模型」页，原因说在那里，面板里不解释。
6. 没保存密钥或没选模型时：开关是灰的，鼠标停住能看到原因，下面有「去「模型」页配置」。
7. 关主窗口：第一次弹一次「还在菜单栏里」，之后不再弹；进程仍在。点 Dock 图标窗口回来。
8. 面板里点「退出」：应用退出；`launchctl print gui/$(id -u)/com.zhengjiaqiao.symsync.gateway` 仍显示路由在跑，Codex 里的第三方模型照常可用。
9. 右键点菜单栏图标：出原生菜单，只有「打开 SymSync」和「退出」。
