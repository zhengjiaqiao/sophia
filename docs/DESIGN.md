...
需要确认一道的开关（开启自动同步 / 自动添加）：先拨过去，弹确认；用户取消就拨回来。开关不因为要确认就退化成按钮。
...
3. **真会丢东西、需要确认一道**：删本体、开启自动同步 / 自动添加、MCP 的批量或跨域写入、重启 Codex——与「页面还是弹层」那四处确认同源
...
**确认只给这四件事，且只确认一道**：删本体（真会丢内容）、开启自动同步 / 自动添加（一次批量建几十条，规模用户看不见）、MCP 的**批量或跨域**写入（跨域会把请求头和令牌一并复制过去）、**重启 Codex**（会结束正在运行的进程）。开关链接、清除失效链接、**MCP 同域的单格写入**都不确认。
...
| 右侧副标题 | 无 | 无 |
| 来源栏列头 | `来源` / `未添加` | `来源` / `未添加` |
| 顶部规则（开关） | `此来源新出现的 skill 自动添加` | `此位置新出现的服务自动添加` |
