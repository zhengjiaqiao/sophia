...
- **2026-09-21 · 一个动词、开关、图标**：① 「导入 / 引入」全改「添加」，页签改名词 `MCP`；用户问"要不要统一叫新增"，选「添加」是因为它已经在侧栏和网关页用着、且不和"新出现"撞词。② 「启用」从两态按钮改成开关，同时作废「不用滑动开关」那条 Don't；自动同步 / 自动添加行、添加页的规则勾一并改开关。③ 图标词表：设置 / 改 / 删掉 / 添加 / 关掉 / 返回 / 搜索有图标，其余写字；网关页的 `改` `删掉` 改成铅笔与垃圾桶。④ 按钮不写宾语：`重启 Codex` → `重启`。⑤ 托盘面板砍掉状态两行，与卡片同步。⑥ MCP 添加页文案逐句对齐 skill 添加页，动作条不再出现绝对路径。⑦ 文案里「矩阵」改「列表」。⑧ 顺手修画板代理报的规范自相矛盾：`mono` 行高是 1.6 不是 2.0、`head-cap` 补进 front-matter、「自动引入」残留、壳高度 56 → 84。
