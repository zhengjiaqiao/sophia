...
| 第三行 | 一句 `caption`：状态 + 等宽事实，合成一句。`还没启用 · Codex 0.155 · 路由未安装`。**不再写"选好的 N 个模型点「启用」就会进 Codex 的模型列表"这种教用户操作的话**——按钮就在那儿 |

卡片之间 16。卡片上方**没有**「第三方模型」区域标题和那句介绍；卡片下方**没有**「用第三方模型要知道的」段落——限制说明搬到网关配置页底部，配置第三方模型的地方才是读它限制的地方。
