...
**实现上这意味着：`display` / `nav-cap` / `button-cap` / `micro-cap` 四档 token 自带 `text-transform: uppercase`，专名嵌进去必须逐个用 `<Plain>` 开洞**——`删 WeiboAP 的` 不包就渲染成 `删 WEIBOAP 的`，`添加 skill 到「全局」` 里的「全局」同理。这条踩过两次，第二次是渲染成图才看出来。
...
只有一种形：ghost pill。四个变体：默认、禁用（**必须同时给 `title` 说明原因**，类型上强制）、反色（表示"现在开着"的开关态，现在只剩选择片在用——布尔状态改走开关）、文字链（次级动作）。两档尺寸按所在容器的高度选，不按重要性选。

**按钮上只写动词，不写宾语。** 宾语是它所在的那一行：Codex 卡上写 `重启`，不写 `重启 Codex`；网关行上写 `改`，不写 `改这家网关`。宾语只在按钮离开了它的对象时才回来——空态里的 `添加 skill`、二级页面名 `添加 skill 到「全局」`。

**能画出来的动作不写字。** 一个动作先问有没有人人认得的图形（见「图标」一节的词表）：有，就用图标按钮 `button-icon`；没有——「重启」「添加」「拆开」「忽略」这类——才用文字。这条来自真机反馈"图标用得太少"：一行三个字按钮并排，眼睛要读完才知道哪个是哪个，图标一眼就分。

### 图标按钮 `button-icon`

16px 图形、32×32 命中区、无描边无底、hover 转 `surface` 底、零圆角（它不是 pill——pill 是"文字按钮"的形，图标按钮的可点性由图形本身承担，设置齿轮就是这么做的）。**必须带 `title`**（同时是 `aria-label`）：图标省下的字放这里。
...
曾经的 Don't 写「不用滑动开关（iOS 语言，必带圆角）」，作废：pill 圆角在这套系统里本来就是「可点」的记号，开关正好落在这个语义上。
...
**确认只给这三件事，且只确认一道**：删本体（真会丢内容）、MCP 的**批量或跨域**写入（跨域会把请求头和令牌一并复制过去）、**重启 Codex**（会结束正在运行的进程）。开关链接、清除失效链接、**MCP 同域的单格写入**都不确认。
...
一行：筛选框（放大镜在框内，占位 `筛选`）+ 来源片 + 右端一个 **`+` 图标按钮**（title `添加 skill` / `添加 MCP`）。**没有「清除失效的（N）」按钮**——失效链接已经逐条进待处理栏、各带 `清除`，工具行再放一个总按钮是同一件事说两遍，而且 N 为 0 时它只是一块灰字。空态里的 `添加 skill` 仍是文字 pill：那时屏幕上没别的东西，一个 `+` 孤零零的没有语境。

自动同步 / 自动添加那一行：`自动同步` 标签 + `通用仓库 → Claude Code · Codex` + `2 条待建` + 右端**开关**（拨开）。第一版右端是 `关掉` 文字 pill，用户反馈"太多文字、没充分用图标"。
...
卡片（hairline 描边、零圆角、内边距 16 24），**两行**：

| 行 | 内容 |
|---|---|
| 第一行 | 左：24px 图标 + 名字（`display` 档，不大写）+ 紧跟一个 `mono` 计数 `103 个可用模型`（`ink-faint`）。右：**开关**（启用）、`重启`（文字 pill，没有宾语）、`配置网关`（文字 pill——它离开这个卡去二级页面，且「配置」单独一个词说不清去哪） |
| 第二行 | **生效的模型**：紧凑片（`chip-compact`），每片带 `×`；整行可点打开选择器；空时写 `还没选模型 · 点这里挑`。片区自然换行 |

**没有第三行。** 第一版第三行写「还没启用 · Codex 0.155 · 路由未安装 · 1 家网关 · 共 103 个模型可挑」——全是我们的内部事实，用户反馈没必要。启没启用**开关本身已经说了**；版本、路由状态、网关几家是运维信息，放配置页。唯一对用户有用的数——**有几个模型可挑**——挪到名字旁边。

例外：`needsCodexRestart` 为真时，行内待办条（§4.4）挂在卡片下面说「改动要重启 Codex 才生效」，动作就是右边那个按钮——这是要用户拿主意的事，不是事实陈述。
...
**网关配置页**：二级页面，同一页两个视图——列表（一家一行）与表单（地址 + 密钥 + `保存`即拉取 + 只读端口协议）。列表一行：状态点 + 名字（`body`）+ 地址（`mono` `ink-faint`）+ `N 个可用模型`（`mono`，这一家网关提供的数；卡上的计数是各家网关去重后的总数——一家网关时两处相等）+ 右端两个**图标按钮**：铅笔（`改`）、垃圾桶（`删掉`）。第一版是两个文字 pill `改` `删掉`，用户反馈这两个都该是图标。区域标题右侧 `添加网关` 文字 pill（它开表单、带宾语，词表里的 `+` 也可以，实现二选一但全应用一致）。底部一段 `caption`：用第三方模型的限制。

**`重启`**：结束这个 agent 的常驻进程，下次启动带新配置。会中断进行中的对话，**确认一道**——确认弹窗里把宾语说全（`重启 <agent> 会结束它正在运行的进程`，agent 名随卡走），按钮仍只写 `重启`。**没有禁用态**——一个进程都没找到不算失败。只有我们知道怎么找到其常驻进程的 agent 才有这个按钮（现在只有 Codex）；画板上第二张卡画了它，是在说多 agent 时的位置。

### 托盘面板

与模型卡**同一张卡的缩小版**：一行 `24px 图标 + Codex + 开关`，下面**没有状态句、没有版本与路由那行**——和卡片砍掉第三行是同一条反馈。`needsCodexRestart` 时挂一条行内待办条（`改动要重启 Codex 才生效` + `重启`），确认在面板里当场展开。菜单三项：`打开 Sophia` `设置` `退出`。
...
| 右侧副标题 | 无 | 无 |
| 来源栏列头 | `来源` / `未添加` | `来源` / `未添加` |
| 列表头 | `<来源> · N 个本体` + `全选` | `<位置> · N 个服务` + `全选` |
| 底部区域标签 | `新添加的 skill 自动在以下 agent 开启` | `新添加的服务自动写进以下位置` |
| 底部规则（开关，在 agent / 位置片下面一行） | `此来源以后新出现的 skill 也自动添加` | `此位置以后新出现的服务也自动添加` |
| 底部动作条左 | `只建链接，不动源文件` | `只新增，不覆盖同名配置` |
| 底部动作条右 | `添加 N 个` | `添加 N 个` |

**页面只有两段**：中间是这次要添加的 skill，底部是"添加到哪"——agent 片和"以后的也自动"那个开关放在一起，因为它们说的是同一组目标（规则的目标就是这些 agent）。第一版把开关单独放在最上面，页面成了三段，用户反馈"过于复杂"。

**规则只管以后的。** 拨开开关不会把来源里现有的 skill 批量添加——现有的就在中间那个列表里，勾选点 `添加 N 个` 即可。实现上规则要记住开启那一刻来源里已有的 skill，之后扫描只对新出现的建链接。这样开关就没有"一次建几十条"的规模问题，**不再需要确认**；自动同步行的 `N 条待建` 也只数新出现的。

来源栏宽度随名字走：MCP 的位置名（`Claude Code · Local settings`）比 skill 来源名长得多，那一页来源栏 380 而 skill 页 268——「逐句对称」说的是话，不是列宽；截成 `Claude Code …` 两行一样就违背了「来源是用户认得的名字」。

**没有 `取消`**——页头已经有 `←`，同一页两个「回去」是多余的；这条来自真机反馈，「关闭 / 取消不是动作」那节早就定了，实现漏了。来源栏底部 `添加来源` 是文字链（与侧栏 `添加项目` 同一档，没有省略号）。动作条里**不出现绝对路径**（skill 页曾写 `源文件在 /Users/…/.agents/skills`，同样删）——MCP 页曾写 `这份定义来自 /Users/…/.claude.json`，路径放来源行的 `title`。第一版 MCP 页的「来源位置 / 能补齐」「这个位置以后新增的服务，自动写进下面选中的位置」「写进以下位置」「把这些服务加进「全局」」全是另一套话，用户反馈应照 skill 页写。
