...
| 行 | 内容 |
|---|---|
| 第一行 | 左：24px 图标 + 名字（`display` 档，不大写）+ 紧跟一个 `mono` 计数 `103 个可用模型`（`ink-faint`）。右：三个动作——开关（ghost pill 两态，反色＝已启用）、`重启 <agent>`、`配置网关` |
| 第二行 | **生效的模型**：紧凑片（`chip-compact`），每片带 `×`；整行可点打开选择器；空时写 `还没选模型 · 点这里挑`。片区自然换行 |

**没有第三行。** 第一版第三行写「还没启用 · Codex 0.155 · 路由未安装 · 1 家网关 · 共 103 个模型可挑」——全是我们的内部事实，用户反馈没必要。启没启用**按钮本身已经说了**（`启用` vs 反色的 `已启用`）；版本、路由状态、网关几家是运维信息，放配置页。唯一对用户有用的数——**有几个模型可挑**——挪到名字旁边。
