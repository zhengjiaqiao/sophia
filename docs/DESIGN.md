...
**网关配置页**：二级页面，同一页两个视图——列表（一家一行）与表单（地址 + 密钥 + `保存`即拉取 + 只读端口协议）。列表一行：状态点 + 名字（`body`）+ 地址（`mono` `ink-faint`）+ `N 个可用模型`（`mono`，与卡上的计数同一口径）+ 右端两个**图标按钮**：铅笔（`改`）、垃圾桶（`删掉`）。第一版是两个文字 pill `改` `删掉`，用户反馈这两个都该是图标。区域标题右侧 `添加网关` 文字 pill（它开表单、带宾语，词表里的 `+` 也可以，实现二选一但全应用一致）。底部一段 `caption`：用第三方模型的限制。

**`重启`**：结束这个 agent 的常驻进程，下次启动带新配置。会中断进行中的对话，**确认一道**——确认弹窗里把宾语说全（`重启 Codex 会结束它正在运行的进程`），按钮仍只写 `重启`。**没有禁用态**——一个进程都没找到不算失败。
