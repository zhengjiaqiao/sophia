...
没有 `取消`（`←` 或 Esc 回）。动作条里**不出现绝对路径**——MCP 页曾写 `这份定义来自 /Users/…/.claude.json`，路径放来源行的 `title`。第一版 MCP 页的「来源位置 / 能补齐」「这个位置以后新增的服务，自动写进下面选中的位置」「写进以下位置」「把这些服务加进「全局」」全是另一套话，用户反馈应照 skill 页写。
