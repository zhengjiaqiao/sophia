...
红绿灯浮在窗口左上角那 28px 里——**那一排留空**，只当拖动区；字标与页签在它下面的 56px 里。第一版把字标和红绿灯挤在同一排，用户反馈"太靠上"。顶栏总高 84 = 28 空 + 56 内容，整条可拖动窗口。
...
**一行一个 agent，生效的模型就在这一行里。** 以后会有多个 agent，每个只能占一行，不能一个 agent 摊成"身份一块、模型一块"两段。

行的结构，从左到右：

| 位置 | 内容 |
|---|---|
| 左 | 24px 图标 + 名字（`display` 档，不大写） |
| 中 | **生效的模型**：紧凑片（`chip-compact`，hairline 不反色），每片带 `×`；整块可点打开选择器；空时写 `还没选模型`。片区自然换行，行高随之长 |
| 右 | 三个动作：开关（ghost pill 两态，反色＝已启用）、`重启 <agent>`、`配置网关` |

行下面一行 `caption`：一句人话状态 + 等宽事实（版本、路由），**合成一句**——`enabled` / `router.running` / `codex.version` 是正交的三件事，不并排三个徽标。网关那句事实（`1 家网关 · 共 103 个模型可挑`）并进这一行末尾。

**没有单独的「生效的模型」区块和标题。** 第一版把它做成行下面一段带区域标签的独立区，一个 agent 占了两段——多 agent 时页面会变成长条。

**选择器**：搜索框 + 列表，已选置顶，已选标记用 **12px 方形复选框**——圆＝状态（只读事实），方＝选择（我选的）。整行可点，勾选当场写盘，没有保存/关闭按钮。

**网关配置页**：二级页面，同一页两个视图——列表（一家一行）与表单（地址 + 密钥 + `保存`即拉取 + 只读端口协议）。
...
- **2026-09-21 · 确认**：删本体、开启自动同步 / 自动引入、MCP 的批量或跨域写入、重启 Codex，四件事各一道。原先写的是「两处」却列了三条，还写着「写入 MCP 配置都不确认」——与 `docs/specs/2026-09-21-ui-rebuild-mcp.md`「同域单格不确认、批量与跨域确认」的裁决相抵；实现按 MCP 那份做的，这里按实现校正。
