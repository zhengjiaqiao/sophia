...
3. **真会丢东西、需要确认一道**：删本体、开启自动同步、重启 Codex——与「页面还是弹层」那三处确认同源
...
**右侧三个动作**：`配置`（二级页面）、开关（ghost pill 两态，反色＝已启用）、**`重启 Codex`**。重启的是 Codex 的常驻进程（`codex app-server`），下次用 Codex 时会带着新配置起来。它会中断进行中的对话，**确认一道**。**没有禁用态**——一个进程都没找到不算失败，提示条说「Codex 现在没在跑，下次启动就是新配置」。
