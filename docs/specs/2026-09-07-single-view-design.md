...
- 域页：工具栏（本域待同步 N / 坏链 K / 同步本域 / 清理本域坏链 / 刷新）；"已引入来源"标签行（每个标签：label，`All` 或 `n 个 skill`，"编辑"、"移除"）；仅有链接、未引入的来源也在标签行显示为「已链接 n 个」，可编辑引入，不可移除；"引入来源…"按钮打开弹层；表格（行首勾选、skill、本体位置、各目标列，可排序）；坏链表；结果提示。
