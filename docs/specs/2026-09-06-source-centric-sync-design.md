...
- 日期：2026-09-06
- 状态：已实现（PR #4）；2026-09-07 起被 `2026-09-07-single-view-design.md` 替代
...
- 按域视图：按 `TargetScope` 分组（全局一组，每个项目一组），表格行 = 本域列出的 (source, skill)，多一列"本体位置"。本域列出的本体位置 = 属于该域的（全局：通用仓库、harness 全局目录、harness 附加目录、手动；项目：该项目的 `.agents/skills`）∪ 在本域目标里有 `Linked` 格子的，属于该域的排前面，其余按 `sources` 原序；外来本体位置以"已链接"为准，`Foreign` / `Broken` / `Duplicate` / `Unwritable` 都不算落在本域里（否则同名 skill 会把每个来源都列进来）；目标集合里勾了本域目标但还没链接的不算，同步出链接后自会以"已链接"出现。列出即视为对本域合法：列出的本体位置显示其全部 skill 行，其在本域目标下的 `Missing` 格子（skill 未被禁用的）都算本域待同步，哪怕本域目标还不在它的目标集合里；"同步本域"先把本域全部目标并进这些本体位置的目标集合，再链接落在本域目标下的缺口，同步集因而与刚建出的链接一致。本视图没有本体位置级复选框，跨域取舍（不想同步进某个域）在按本体位置视图处理。每行行首有一个 skill 复选框（`set_skill_enabled`），与本体位置视图的行级复选框同源，禁用行淡显。表格下方列出本域目标里的坏链，清理按钮在顶部工具条。
- 两种视图的表头可点击排序（skill 名、本体位置、各目标列按状态）。
- 顶部工具条跟着侧栏选中项走：本体位置视图统计并同步落在该本体位置下的待同步项，域视图统计并同步落在本域目标目录下的待同步项与坏链，其后以淡显文字给出全局待同步数。
- 任何勾选或动作后重新 `scan_all`。
