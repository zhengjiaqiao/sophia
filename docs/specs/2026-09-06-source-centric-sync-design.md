...
- 按域视图：按 `TargetScope` 分组（全局一组，每个项目一组），表格行 = 属于该域的本体位置的 (source, skill)（全局：通用仓库、harness 全局目录、harness 附加目录、手动；项目：该项目的 `.agents/skills`）；跨域同步在本体位置视图处理，多一列"本体位置"。表上方每个本体位置一个复选框：取消 = 从该本体位置的目标集合里去掉本域全部目标，勾上 = 加回；取消后该本体位置的行不再显示；部分勾选显示半选；与本体位置视图共用同步集。
- 任何勾选或动作后重新 `scan_all`。
