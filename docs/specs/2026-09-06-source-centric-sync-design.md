...
- 按域视图：按 `TargetScope` 分组（全局一组，每个项目一组），表格行 = 本域内的 (source, skill)，跨域同步在本体位置视图处理，多一列"本体位置"。本域的本体位置 = 属于该域的（全局：通用仓库、harness 全局目录、harness 附加目录、手动；项目：该项目的 `.agents/skills`）∪ 在本域目标里有非 `Missing` 格子的 ∪ 目标集合含本域任一目标的，属于该域的排前面，其余按 `sources` 原序。行的显隐：仅当该本体位置在本域未勾选、且它在本域的格子全是 `Missing` 时才隐藏，只要有一个非 `Missing` 格子就照常显示。表上方每个本体位置一个复选框：取消 = 从该本体位置的目标集合里去掉本域全部目标，勾上 = 加回；部分勾选显示半选；与本体位置视图共用同步集。
- 任何勾选或动作后重新 `scan_all`。
