...
- 按域视图：按 `TargetScope` 分组（全局一组，每个项目一组），表格行 = 本域内的 (source, skill)，跨域同步在本体位置视图处理，多一列"本体位置"。本域的本体位置 = 属于该域的（全局：通用仓库、harness 全局目录、harness 附加目录、手动；项目：该项目的 `.agents/skills`）∪ 在本域目标里有 `Linked` 格子的 ∪ 目标集合含本域任一目标的，属于该域的排前面，其余按 `sources` 原序；外来本体位置以"已链接"为准，`Foreign` / `Broken` / `Duplicate` / `Unwritable` 都不算落在本域里（否则同名 skill 会把每个来源都列进来）。行的显隐：该本体位置在本域未勾选时，仅当该行在本域有至少一个 `Linked` 格子才显示；已勾选（全选或半选）的本体位置照常显示全部行。表上方每个本体位置一个复选框：取消 = 从该本体位置的目标集合里去掉本域全部目标，勾上 = 加回；部分勾选显示半选；与本体位置视图共用同步集。
- 每张卡片显示该本体位置的待同步数并可单独同步；顶部按钮为同步全部。
- 任何勾选或动作后重新 `scan_all`。
