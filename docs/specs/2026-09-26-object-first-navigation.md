...
- **位置列**：只在位置集合大于 1 时出现，面板宽仍 776：地方够时位置 88、来源照常 144，agent 列多了先收来源到 80、再收位置到 72，先保住名称列 246（4 个 agent 时名称 238，MCP 5 个 agent 列时名称 150）；显示「用户级」或项目名（同名项目靠短路径区分，规则同「更多」列表）。
