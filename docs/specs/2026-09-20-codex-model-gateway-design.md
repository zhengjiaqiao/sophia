...
## 验证记录（验证时点：2026-09-20 22:00；Codex 桌面应用 26.915，内核 0.155.0-alpha.9.2；macOS 26.5.2）

**核心链路还没有在最终环境验证。** 本机的 Codex 目前仍由 agents-manager（Go）接管，SymSync 的真实接管（AC22）没有执行，所以“Codex 桌面应用经 SymSync 的路由使用两类模型”这件事只在隔离目录里、用桌面应用自带的 Codex 内核验证过。没有执行的原因见文末。另外，发起人的官方额度在验证期间用尽（2026-09-21 12:10 恢复），需要官方模型完成对话的条目只验证到“请求到达官方后端并原样带回了额度提示”。

计数：通过 9，代理通过 20，未验证 1，失败 0。

| 编号 | 验证 | 结果 |
|---|---|---|
| AC1 | 调试版应用包里实看：导航有「模型」，页面有网关、模型、Codex 三块 | 通过 |
| AC2 | macOS 专属代码均有 `cfg` 门控；本机没有 Linux 工具链，未实际编译，等 CI | 代理通过 |
| AC3 | Rust 生成的合并目录被桌面应用自带的 Codex 内核接受，`codex exec -m thudm-glm-5.2` 能选中。桌面应用选择器未看（未接管） | 代理通过 |
| AC4 | 真实内核 → launchd 拉起的 Rust 路由 → 真实 wecode：GLM-5.2、DeepSeek V4 Flash 流式回复，日志去向 third_party。桌面应用界面内未验 | 代理通过 |
| AC5 | 上午：真实内核经 Rust 路由（终端启动）与官方模型完成对话和一次读文件；下午：经 launchd 里的路由到达官方后端，额度用尽的 429 提示原样回到 Codex。字节级透传由单测保证 | 代理通过 |
| AC6 | 真实内核 + 真实 wecode：GLM-5.2、DeepSeek V4 Flash 发起工具调用读出文件内容 | 代理通过 |
| AC7 | 单测覆盖官方路径清理；真实的“第三方 → 官方 → 第三方”同会话切换因官方额度用尽未做（Go 版同样的逻辑当天验证过） | 代理通过 |
| AC8 | 对真实网关经 Rust 路由重放：回给客户端恰好一个压缩条目，带着它续聊答出了摘要里的端口号 | 通过 |
| AC9 | 真实二进制 curl：大小写不同的 model 键 400、gzip 415；停用模型、变体、数组、重复键、BOM 由集成测试覆盖 | 代理通过 |
| AC10 | 真实二进制 curl：带 Origin 403、伪造 Host 403；`Sec-Fetch-Site` 与非回环来源由测试覆盖 | 通过 |
| AC11 | 演练全程没有开 SymSync 界面，路由由 launchd 运行，真实内核的请求正常。桌面应用未验 | 代理通过 |
| AC12 | 真实 launchd：`kill -9` 后 0.2 秒恢复；另做 3 轮“重启后立刻发第三方请求”全部成功 | 通过 |
| AC13 | launchd 里的路由进程环境中没有代理变量：wecode（在系统代理例外列表里）直连成功，官方后端经系统代理可达 | 通过 |
| AC14 | 需要重启电脑 | 未验证 |
| AC15 | `crates/core/tests/codex_models_with_mcp.rs` 两个方向；界面走查未做 | 代理通过 |
| AC16 | 同上文件：过期的 MCP 预览、过期的模型页快照都被拒绝且不覆盖 | 代理通过 |
| AC17 | `make test-core`：现有 MCP 测试文件 diff 为空、全部通过，备份名仍是 `config.mcp.bak` | 通过 |
| AC18 | 隔离演练：真实二进制对真实设置的副本启用，diff 恰好两行，备份为 `config.models.bak`；单测断言登录凭据文件未被触碰。真实设置未写（未接管） | 代理通过 |
| AC19 | `ac19_*` 单测 | 代理通过 |
| AC20 | 隔离演练：路由不通时恢复成功，设置与原件 `cmp` 一致，本功能文件与后台服务清除；单测覆盖默认模型回退 | 代理通过 |
| AC21 | `ac21_*` 单测 | 代理通过 |
| AC22 | `ac22_*`、`takeover_failure_*` 单测。**真实接管未执行** | 代理通过 |
| AC23 | 真实应用包里实看：识别出 agents-manager、给出接管提示、启用不可用并说明原因；agents-manager 的文件和服务未被改动 | 通过 |
| AC24 | `ac24_*` 单测；命令层先校验后提交 | 代理通过 |
| AC25 | 演练后扫描 135 个文件（演练目录、SymSync 数据目录、LaunchAgents、仓库源码与文档），无密钥明文；plist 参数里没有密钥 | 通过 |
| AC26 | 隔离演练：端口被别的程序占用时状态给出原因，再次启用失败且设置未动，`symsync gateway restore` 成功；横幅由视图逻辑测试覆盖 | 代理通过 |
| AC27 | 单测（含“内容没变的重复启用不提示”）；演练中 `doctor` 的提示符合预期 | 代理通过 |
| AC28 | 真实 launchd：换一个新构建后执行启用，后台副本与新构建逐字节一致，路由进程号改变，设置未动 | 通过 |
| AC29 | `make test`：302 个 Rust 测试通过，clippy 零警告，前端构建通过，18 个前端逻辑测试通过 | 通过 |
| AC30 | `ac30_proxy_overhead_p95_under_50ms`（本机假上游，50 次请求）。未在真实链路上计时 | 代理通过 |

证据：`make test`（321 个 Rust 测试、clippy 零警告、前端构建、18 个前端逻辑测试）；演练目录下的路由日志；真实运行的命令输出见会话记录。

真实环境里发现并处理的：`toml_edit` 做不到逐字节还原（改用文本级手术）；进程重启后见过一次瞬时连接失败（复现不了，加了“连接未建立时重试一次”）；Kimi K2.5 偶尔只输出思考内容（沿用 Go 版的兜底）。

**独立验证（2026-09-20，只读的第二个代理）。** 没有发现高危问题和凭据泄漏，并确认 Go 版加固过的防护在 Rust 版里都在。它发现并已修复、都有回归测试的：恢复后再启用会忘掉已停用的模型（Go 版同样如此，已一并修）；非 POST 或非 JSON 声明的请求解析失败时放行到官方；zstd 只解第一帧；非 ASCII 的 `Content-Encoding` 被当成没压缩；来源校验晚于读请求体、读请求头没有超时；流转换的行缓冲是平方复杂度且无上限；后台副本复制用固定临时文件名、完全信任记录的指纹；接管失败后留下自己的服务和文件、过早覆盖密钥；路径穿越；拉模型时持锁跨网络请求；`main.rs` 遇到非 UTF-8 参数会 panic；状态接口回显未清洗的模型名；网关错误体里回显的密钥被原样转给客户端。

没有修、属于已知限制的：全角字符、西里尔形近字母这类模型名变体仍会被当成“不认识的模型”放行到官方（Codex 自己只会发目录里的标识，Go 版同样如此，没有通用解法）；自动审阅的拦截依赖请求头里的会话标识，属于尽力而为；没有移植“无请求体的 GET 在 HTTP/2 上游仍无请求体”那条测试（需要带 TLS 的 HTTP/2 测试上游），该行为在真实官方上游上验证过。

**终审（2026-09-20，软链接同步会话）：修后合并，无 Critical。** 5 条 Important 已全部修复并带回归测试：
读取系统代理时子进程不退出会永久挂起（读线程持锁等待，超时分支拿不到锁；挂在常驻进程里官方与第三方模型同时不可用）；
取系统代理时持锁跨子进程调用，并发请求被串行；同步 MCP 命令里 `blocking_lock` 的注释理由不对（不会 panic，但会占住 IPC 线程）；
模型页六个命令把写锁持到状态查询结束，放大了上面的卡顿；接管在写设置之前失败时留下“密钥已覆盖 + 服务在跑 + 文件已落盘”却不清理；
`security -i` 失败时把它的输出回传进错误信息，而输入里含编码后的密钥。
另外处理了三条 Minor：密钥回显兜底补到原样转发的第三方路径；`launchctl` 改用绝对路径；编排层的锁中毒不再让整个功能瘫痪。

**为什么没有执行真实接管。** 发起人日常运行的是已安装的旧版 SymSync，它不认识 `settings.json` 里新增的 `codexGateway` 字段：读取时会忽略，**保存时会丢掉**。如果现在用开发构建接管，之后旧版 SymSync 任何一次保存设置（开关 harness、改自动同步规则）都会抹掉网关设置，而 Codex 设置仍指向路由，状态就此错位。所以接管应当在这个分支合并、发起人换上新构建之后再做。这同时是一条发布注意事项：**带模型页的版本一旦启用过，就不要再回退到旧版 SymSync 去改设置。**
