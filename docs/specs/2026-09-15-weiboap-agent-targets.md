...
## 修订 v2（2026-09-15，实现后回退）

**撤销 R2、R3、R5，以及为它们引入的「一列多目录」模型。**

理由：全局域的扇出列会把一列写进各个助手（项目）域的目录，实际上是"全局往项目同步"——这条产品边界早在 skill 同步立项时就定为不做。每个助手已经是独立的域，要给它同步就在它自己的页面走引入逻辑，这才和其他项目一致。扇出列让"域"的含义变得可疑，收益不抵。

保留与撤销：

| 需求 | 处置 |
|---|---|
| R1 托管目录 | **保留**。WeiboAP 的 custom 缓存目录只作本体位置，不生成任何可写列 |
| R2 一列多目录 / 全局扇出列 | **撤销**。`Target` 恢复单目录 `path`；全局域不再出现 WeiboAP |
| R3 部分覆盖 `Partial` | **撤销**。没有多目录列就没有部分覆盖；`CellState::Partial`、`Cell.linked/total` 一并移除 |
| R4 助手域单写 | **保留**（本就如此）。给某个助手同步，在它自己的域页面点引入 |
| R5 新助手自动补齐 | **撤销**。`fan_out_cells` 移除 |
| R6 助手名 | **保留** |
| R7 清理残留 | **已完成** |

一并保留的两处修复（与扇出无关，独立有价值）：
- `exclude` 在本体位置尚无规则时新建一条只有排除名单的规则；
- `remove_auto_link_targets` 仅在 targets 与 excluded 都空时才整条删除。

回退后 WeiboAP 在界面上的最终形态：全局域看不到 WeiboAP 列，但 custom 里的官方 skill 作为本体位置成行，可同步到 Claude Code、Codex 等；4 个助手各是一个域，各有一列，可在其中引入与清除。

对应撤销的验收标准：AC3–AC11 作废；AC1、AC2、AC12–AC16 继续有效。
